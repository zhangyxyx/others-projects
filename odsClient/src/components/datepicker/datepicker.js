...
class DatePicker extends React.Component{
    constructor(props){
        super(props);
        var onSelectDateDay=Number(moment(props.defaultvalue).format("DD") || moment().format("DD"));//初始天
        this.state={
    ...
    monthDay(initMonth,onSelectDateDay){
        ...
        for(var j=0;j<lineNum; j += 1){
            daysArr[j]=[];
            for(var i = 0; i < 7; i += 1){
                var virtualDay = (lastMonthDays - currentWeekday) + i,
    ...
    reRenderCalender(initMonth,yearTitle,monthTitle){
        var  calendarData=this.monthDay(initMonth,this.state.onSelectDateDay);
        this.setState({calendarData,initMonth,yearTitle,monthTitle})
    }
    turnYearMonth(y=0,m=0){
        var month=this.state.monthTitle-1,
    ...
    onSelectDate(e,day){
        ...
        var {yearTitle,monthTitle}=this.state,
        ...
        onDateChange && onDateChange(onSelectDateVal,this.props.sendField);
    }
    onSelectToday(){
        var onSelectDateVal=moment().format("YYYY-MM-DD"),
        ...
        return(
            <div className="datepicker" onClick={(e)=>{e.nativeEvent.stopImmediatePropagation()}}>
                <div onClick={this.showDatePicker}>
                    <input placeholder={placeholder ? placeholder : "请选择日期"} value={onSelectDateVal} readOnly/>
                        ...
                        <div className="datepicker-plate">
                            ...
                            <ul className="plate-header">
                                <li className="leftArea">
                                    <span className="arrowYear" onClick={()=>{this.turnYearMonth(-1,0)}}/>
                                    <span className="arrowMonth" onClick={()=>{this.turnYearMonth(0,-1)}}/>
                                </li>
...
class MonthPickerPlate extends React.Component{
    ...
    turnYear(i=0,type){
        switch (type){
            ...
            default:{
                var cureentYearTitle=Number(this.state.cureentYearTitle)+i;
                this.setState({cureentYearTitle})
            }break;
        }
    }
    componentWillMount(){
        var {showYearPicker,yearTitle}=this.props;
        showYearPicker && this.showYearPlate(yearTitle);
    }
    showYearPlate(cureentYear){
        var {cureentYearTitle}=this.state,
        ...
        for (var i=0;i<4;i++){
            ...
            for(var j=0;j<3;j++){
                if(index>9){break;}
                var yaerConcat=cureentYearCut+index;
    ...
    onSelected(select,type){
        switch (type){
            ...
            default:{
                var {showYearPicker,onSelectYearMonth}=this.props;
...
class EachDay extends React.Component{
    ...
    render(){
        var {dayDetail,onSelectDate,onSelectDateDay}=this.props;
        switch (dayDetail){
